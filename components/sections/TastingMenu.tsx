"use client";

import { site } from "@/data/site";
import { tastingMenu } from "@/data/menus";
import { images } from "@/data/images";
import { contact } from "@/data/contact";
import { useScroll } from "@/components/providers/SmoothScroll";
import { ImageReveal } from "@/components/ui/ImageReveal";
import { WordReveal } from "@/components/ui/WordReveal";
import { Reveal } from "@/components/ui/Reveal";
import { Button } from "@/components/ui/Button";
import { Eyebrow } from "@/components/ui/Eyebrow";

/**
 * The tasting menu, framed so the page never depends on which menu is
 * currently running: what it is, how often it changes, and one way to find
 * out what is on the table right now.
 */
export function TastingMenu() {
  const { scrollTo } = useScroll();
  const wa = contact.whatsapp;

  return (
    <div id="tasting-menu" className="scroll-mt-20">
      <div className="grid grid-cols-12 gap-x-6 gap-y-12">
        <div className="col-span-12 lg:col-span-6">
          <Eyebrow rule className="text-orange-deep">
            {tastingMenu.meta}
          </Eyebrow>
          <WordReveal as="h3" text={tastingMenu.title} className="t-section mt-6 text-indigo" />
          <Reveal className="mt-8">
            <p className="t-quote text-ink-soft">{tastingMenu.subtitle}</p>
          </Reveal>
          <Reveal stagger={0.12} className="mt-10 grid gap-5">
            {tastingMenu.lines.map((l, i) => (
              <p key={i} className={i === 0 ? "t-lead max-w-lg text-ink" : "t-body max-w-prose text-ink-soft"}>
                {l}
              </p>
            ))}
          </Reveal>
        </div>

        <div className="col-span-12 md:col-span-7 md:col-start-6 lg:col-span-5 lg:col-start-8">
          <ImageReveal
            image={images.communalTable}
            sizes="(min-width: 1024px) 40vw, (min-width: 768px) 58vw, 100vw"
            className="aspect-[4/3] w-full"
            from="right"
            parallax={5}
            cursor="view"
          />

          {/* What's on the table right now — deliberately not menu content */}
          <Reveal className="mt-10 border border-line bg-paper-deep p-7 md:p-9">
            <p className="t-caption text-ink/60">{tastingMenu.currentPrompt.line}</p>
            <p className="t-title mt-3 text-indigo">{tastingMenu.currentPrompt.heading}</p>
            <div className="mt-7 flex flex-wrap items-center gap-x-8 gap-y-4">
              <Button
                href="#contact"
                variant="primary"
                cursor="reserve"
                onClick={(e) => {
                  e.preventDefault();
                  scrollTo("#contact", { offset: -64 });
                }}
              >
                Register your interest
              </Button>
              {wa ? (
                <Button href={`https://wa.me/${wa}`} external variant="text">
                  WhatsApp us
                </Button>
              ) : (
                <Button href={site.links.instagram} external variant="text">
                  Message us on Instagram
                </Button>
              )}
            </div>
          </Reveal>
        </div>
      </div>
    </div>
  );
}
