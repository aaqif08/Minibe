 ...
 */
export function NextPage({ href, label, note, light, className }: Props) {
  return (
    <div className={cn("wrap", className)}>
      ...
      >
        <span>
          <span className={cn("t-eyebrow block", light ? "text-paper/70" : "text-ink/70")}>Next</span>
          <span
            className={cn(
          ...
          {note ? (
            <span className={cn("t-body-sm mt-3 block max-w-md", light ? "text-paper/60" : "text-ink-soft")}>{note}</span>
          ) : null}
        </span>
        <span
          aria-hidden
